...
async function collectRoutes (type) {
  ...
  this[`${type}InvRoutes`] = []
  const items = ['']
  if (this.app.waibuStatic) items.push('waibuStatic.asset', 'waibuStatic.virtual')
  if (this.app.waibuRestApi) items.push('waibuRestApi')
  if (this.app.waibuMpa) items.push('waibuMpa')
  const pattern = `{${items.join(',')}}@${type}-routes.*`
  await eachPlugins(async function ({ file, ns, dir }) {
    await collect.call(this, type, { file, ns, dir })
